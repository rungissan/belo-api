      ...
      function(client, subject, scope, done) {
        ...
        if (subject) {
          models.users.findByUsernameOrEmail(subject, function(err, user) {
            ...
            if (!user) {
              return done(new AuthorizationError(`Invalid subject: ${subject}`, 'access_denied'));
            }
            models.permissions.isAuthorized(client.id, user.id, scope,
              function(err, authorized) {
                ...
                if (authorized) {
                  ...
                  return done(new AuthorizationError(`Permission denied by ${subject}`, 'access_denied'));
                }
              });
          });
        } else {
        ...
        function(err, authorized) {
          if (err) {
            ...
            server._respond(req.oauth2, res, function(err) {
              ...
              return next(new AuthorizationError(`Unsupported response type: ${req.oauth2.req.type}`,
